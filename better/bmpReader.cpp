...
unsigned char* BmpReader::ReadBMP(char const *f, int *a_width, int *a_height )
{
	...
	for (int y = 0; y < height; y++) {
		fread(image_s + width * byte_per_pixel * y, sizeof(unsigned char), (size_t)(long)width * byte_per_pixel, fp_s);
		for (int p = 0; p < pad; p++) {
			fgetc(fp_s);
		}
	}
...
int BmpReader::WriteBMP(char const *f, int a_width, int a_height, unsigned char *ptr )
{
	...
	file_size = row * height + rgb_raw_data_offset;
	...
	// write image
	for (int y = 0; y < height; y++) {
		fwrite(ptr + width * byte_per_pixel * y, sizeof(unsigned char), (size_t)(long)width * byte_per_pixel, fp_t);
		for (int p = 0; p < pad; p++) {
			fputc(0, fp_t);
		}
	}
